"""
Command-Line Interface for IntentStack: Hierarchical Sub-Dialog & Multi-Turn Interruption Resumer Agent.
"""
import argparse
import csv
import json
...
from .agents import IntentStackCoordinator

coordinator = IntentStackCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="dialogue-intent-stack-manager", description="IntentStack: Hierarchical Sub-Dialog & Multi-Turn Interruption Resumer Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  INTENTSTACK: HIERARCHICAL SUB-DIALOG & MULTI-TURN INTERRUPTION RESUMER AGENT")
        print(f"  Domain: Autonomous Context Management & State Engines | Standard: Pushdown Automata Dialogue Management")
    ...
    if args.command == "batch":
        try:
            ...
            print(f"Error: Failed to parse CSV '{args.input}': {e}", file=sys.stderr)
            return 1
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting IntentStack: Hierarchical Sub-Dialog & Multi-Turn Interruption Resumer Agent on http://{args.host}:{args.port}")
